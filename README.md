...
> 一个面向本地开发与演示的 AI 招聘辅助系统。项目基于开源招聘 Demo 进行二次开发和架构优化，使用 Vue 3 管理端、Spring Boot 业务后端、FastAPI Agent Runtime 和 RAG Service 构建招聘流程闭环。
...
- Python 3.11，仓库 Dockerfile 使用 Python 3.11；本地测试可使用仓库根目录 `.venv`。
...
如果 `infra\.env` 已存在，不需要重复复制；直接执行 `docker compose ... up -d --build` 即可。
...
```bat
cd java-backend
rem 先将 MYSQL_*、REDIS_* 等变量设置为 infra\.env 中的实际值。
rem 变量说明见 java-backend\README.md。
mvn spring-boot:run
```
...
- `infra/README.md`：基础设施启动、Compose 配置和安全边界。
