...
当前项目已完成工作区初始化、基础设施配置、MySQL 初始化脚本、Java 后端基础业务接口、Python Agent Runtime 结构化接口、DeepSeek 配置化调用、RAG Service Milvus/内存双后端、Vue 管理端，以及 V1 最小闭环所需的服务编排。

实现偏差说明：项目描述中曾要求 Java 数据访问使用 MyBatis；当前 Java 后端实际使用 Spring Data JPA。保留 JPA 的原因是已有业务接口和测试基于 JPA 成型，继续补齐 AI/RAG 闭环的收益高于立即迁移数据访问层。后续如必须对齐 MyBatis，可在保持 REST API 不变的前提下替换 Repository 层。
...
当前已提供 Vue 单页工作台，通过 Java Backend 的 `/api/**` 完成岗位创建、候选人创建、申请创建、简历上传、Agent 分析触发、邮件草稿、面试建议和招聘问答。
...
- 邮件草稿审核页面。
- 面试安排审核页面。
- Agent 调用状态和 RAG 证据展示。

### `frontend/candidate`

候选人端工作区，V1 只保留结构，不优先实现完整功能。
...
Python 智能体运行时工作区，当前已提供 FastAPI 服务、结构化 Agent 接口和 DeepSeek JSON Output 配置化调用；当 DeepSeek 未配置或返回异常时，自动回退到规则化输出。
...
RAG 服务工作区，当前已提供 FastAPI 服务、文本切分、轻量 embedding、内存检索和 Milvus 持久化检索；Docker Compose 默认使用 Milvus，单独本地开发可使用 `auto` 或 `memory` 模式。
...
MySQL 工作区，已放置数据库初始化脚本和岗位模板种子数据。
...
基础设施工作区，已放置本地 Docker Compose 和环境变量示例。
...
DeepSeek 调用由 Python Agent Runtime 统一封装，通过环境变量配置：
...
1. 执行本地基础设施 smoke test，确认 MySQL、Redis、Milvus 可启动和健康。
2. 补充 Java 后端基础工程，先实现岗位、候选人、申请、简历元数据和状态流转。
3. 补充 RAG Service，完成 Milvus collection、文本切分和检索接口。
4. 补充 Python Agent Runtime，先实现 DeepSeek 调用封装和三个 Agent 的结构化输出。
5. 联调 Java Backend、RAG Service、Python Agent Runtime。
6. 补充 `frontend/admin` 管理后台页面。
7. 增加邮件、日历、会议的第一批供应商适配器。
8. 增加 observability 指标、调用审计和错误告警。
9. 增加端到端验收流程：上传简历、分析、生成邮件、生成面试安排。
